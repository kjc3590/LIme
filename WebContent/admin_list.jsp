		...
		<div class="container">
			...
			<div class="card">
				...
				<div class="card-header">
					<h2 class="line"><a href="admin_list.jsp">요청목록</a></h2>
				</div>
					...
					<div class="col-md-4">
						<form name="serach" method="post" class="form1">
							<select class="choice" name="keyField">
				                ...
				                <option value="c.company_name">업체명</option>
				                <option value="a.member_name">요청자명</option>  
				                <option value="a.question_manager">담당자명</option>
				                <option value="a.question_progress">작업진행도</option>
				                <option value="a.question_division">분류</option>
				                <option value="a.question_content">요청내용</option>
							</select> 
							<input type="text" name="keyWord" class="text" /> 
							<input class="btn-Lime btn" type="button" value="검색" onclick="searchCheck(form)" class=" btn btn-primary m-r-10" />
						</form>
					</div>
					<div class="hoho col-md-4">
						<form name="sort" method="post" class="form1">
							<select class="choice" name="sort">
				                ...
				                <option value="a.question_division">분류</option>
				                <option value="a.question_content">요청내용</option>
				                <option value="a.question_day">등록 시간</option>
							</select> 
							<input class="btn-Lime btn" type="submit" value="정렬"  class=" btn btn-primary m-r-10" />
						</form>
					</div>
					<div class="col-md-4">
			...
			try{
				...
				for(Lime resv : resvList){
					...
					String Company_name = resv.getCompany_name();
					...
					if(progress.isEmpty()){
			%>
			  <tr class="warning pdrl12" style="cursor: pointer; " onclick="location.href='admin_detail.jsp?Question_id=<%=resv.getQuestion_id()%>'" >
			...
			%>
			  <tr class="success pdrl12" style="cursor: pointer; " onclick="location.href='admin_detail.jsp?Question_id=<%=resv.getQuestion_id()%>'">
			  	<td ><%=resv.getQuestion_id() %></td>
			  	<td ><%=Company_name %></td>
			  	<td ><%=resv.getQuestion_day() %></td>
			  	<td ><%=resv.getReply_day() %></td>
			...
			%>
			    <li>
			      <a href="admin_list.jsp?pageNum=<%=startPage -1 %>" aria-label="Previous">
			...
			%>
						<li><a href="admin_list.jsp?pageNum=<%=i%>"><%=i%> <span class="sr-only">(current)</span></a></li>
			<%        		
			        }
			}
			...
			%>
				<li>
				<a href="admin_list.jsp?pageNum=<%=startPage + pageBlock%>" aria-label="Next">
